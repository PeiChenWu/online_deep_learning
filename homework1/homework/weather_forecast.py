...
class WeatherForecast:
    ...
    def find_the_largest_drop(self) -> torch.Tensor:
        ...
        return (avg_temps[1:] - avg_temps[:-1]).min()
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        """
        day_means = self.data.mean(dim=1, keepdim=True)
        idx = (self.data - day_means).abs().argmax(dim=1)
        return self.data.gather(1, idx.unsqueeze(1)).squeeze(1)
