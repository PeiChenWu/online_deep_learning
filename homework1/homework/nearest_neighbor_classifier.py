...
class NearestNeighborClassifier:
    ...
    def make_data(cls, x: list[list[float]], y: list[float]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        tensor_x = x if isinstance(x, torch.Tensor) else torch.tensor(x, dtype=torch.float32)
        tensor_y = y if isinstance(y, torch.Tensor) else torch.tensor(y, dtype=torch.float32)

        return tensor_x.clone().detach(), tensor_y.clone().detach()
    ...
    def get_k_nearest_neighbor(self, x: torch.Tensor, k: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        x = self.input_normalization(x)
        idx = torch.topk(torch.norm(self.data_normalized - x, dim=1), k, largest=False).indices 
        return self.data[idx], self.label[idx]
